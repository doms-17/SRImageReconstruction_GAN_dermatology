...
# threshold to classify a border as near-black or not and so to state if there is Dark Corner
DARK_THRESH = 15
...
class ImageArtifact:
    def __init__(self, path):
        ...
        # json_to_dict = json.load(open(
        #     "D:\\DOMI\\University\\Magistrale\\Tesi\\Pipeline_coding\\files_selected_per_label.json"))
        json_to_dict = json.load(open(
            "D:\\DOMI\\University\\Magistrale\\Tesi\\Pipeline_coding\\files_deleted_per_label(valSet).json"))
        # self.files_manually_selected = json_to_dict["files_per_label"]
        self.files_manually_deleted = json_to_dict["files_per_label"]
    ...
    def handle_artifacts(self):
        """Handle artifact:
            check if image has Dark Corner artifact, if not check if it is 1024x1024, otherwise manage the rectangular image
        """
        for label in self.labels:
            new_root_and_dir = create_folder(
                path="", sub_dir="dataset_noArtifact_sliding", folder_name=label)  # create folder per each label
            # retrieve path of preDataset
            root_and_dir = os.path.join(self.path, label)

            for file in tqdm(self.files_per_labels[label]):
                # if file not in self.files_manually_deleted[label]:
                    ...
                    try:
                        ...
                        # If image is neither Dark Corner or squared, then it is rectangular
                            ...
                            if max(height, width)-min(height, width) >= THRESH_RATIO:
                                ...
                                file_reza = f'{id_image}_a.png'
                                file_rezb = f'{id_image}_b.png'
                                file_rezc = f'{id_image}_c.png'

                                new_img_filename = os.path.join(
                                    new_root_and_dir, file_reza)
                                cv2.imwrite(new_img_filename, image_a, [
                                            cv2.IMWRITE_PNG_COMPRESSION, COMPRESSION])
                                new_img_filename = os.path.join(
                                    new_root_and_dir, file_rezb)
                                cv2.imwrite(new_img_filename, image_b, [
                                            cv2.IMWRITE_PNG_COMPRESSION, COMPRESSION])
                                new_img_filename = os.path.join(
                                    new_root_and_dir, file_rezc)
                                cv2.imwrite(new_img_filename, image_c, [
                                            cv2.IMWRITE_PNG_COMPRESSION, COMPRESSION])
                            else:
