...
def handle_artifacts(image_filename):
    ...
    if ((sx_border and dx_border) or (up_border and down_border)
        ...
        if results is not None:
            ...
            final_image = noDarkCorner_image
            final_image = cv2.resize(
...
def test():
    root_test_sets = "D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\TestSet_original\\"
    new_root_test_sets = "D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\TestSet_processed\\"
    ...
    test_toProcess = ["Novara dermoscopio good", "Novara dermoscopio trash"]

    for test_set in test_sets:
        if test_set in test_toProcess:
            new_root_test_set = f"{os.path.join(new_root_test_sets, test_set)}_512"
