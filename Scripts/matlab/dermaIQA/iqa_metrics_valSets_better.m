...
for idx_subfolder = 1 : length(subfolders)
    ...
    if iteration(1) ~= '.'
        ...
        % Metrics
        stats_metrics_tmp = table( str2double(iteration), mean(array_psnr), mean(array_ssim), mean(array_mssim), mean(array_fsim),...
...
tmp_NR = sortrows(stats_metrics, ["niqe_sr","piqe_sr"], "ascend");
tmp_FR = sortrows(stats_metrics, ["ssim","fsim","psnr"], "descend");
best_iterations_NR = tmp_NR(1:5,:);
best_iterations_FR = tmp_FR(1:5,:);
fprintf(1, "\nBest Iteration NR: %d", best_iterations_NR.("iter")(1));
fprintf(1, "\nBest Iteration FR: %d", best_iterations_FR.("iter")(1));
