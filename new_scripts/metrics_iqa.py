...
def pnsr(gt, gan):
    ...
    psnr_index = piq.psnr(gt, gan, data_range=255, reduction='none')
    return psnr_index


def ssim(gt, gan):
    # To compute SSIM index as a measure, use lower case function from the library:
    ssim_index = piq.ssim(gt, gan, data_range=255)
...
def fsim(gt, gan):
    ...
    fsim_index: torch.Tensor = piq.fsim(
        gt, gan, data_range=255, reduction='none')
...
def msssim(gt, gan):
    ...
    ms_ssim_index: torch.Tensor = piq.multi_scale_ssim(gt, gan, data_range=255)
...
def main():
    # path = os.getcwd()
    # path_gt = "D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\results\\test\\61F.png"
    # path_lq = "D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\results\\test\\61F.png"
    # path_gan = "D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\results\\test\\61F_sr.png"
    ...
    gt = torch.tensor(
        skimage.io.imread("D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\results\\test\\61F.png")).permute(
            2, 0, 1)[None, ...]
    lq = torch.tensor(
        skimage.io.imread("D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\results\\test\\61F.png")).permute(
            2, 0, 1)[None, ...] / 255.
    gan = torch.tensor(
        skimage.io.imread("D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\results\\test\\61F_sr.png")).permute(
            1, 2, 0)[None, ...]

    gan = read_image("D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\results\\test\\61F_sr.png").permute(1,2,0)
    ...
    gt_esr = cv2.imread("D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\results\\test\\61F.png")
    gan_esr = cv2.imread("D:\\DOMI\\University\\Thesis\\Coding\\Dataset\\results\\test\\61F_sr.png")
    psnr_gt_2 = basicsr.metrics.calculate_psnr(gt_esr, gan_esr, crop_border=0)
    ssim_gt_2 = basicsr.metrics.calculate_ssim(gan, gan_esr, crop_border=0)
    # piqe_gt = basicsr.metrics.calculate_niqe(gan, crop_border=0)
    # piqe_gan = basicsr.metrics.calculate_niqe(gan_esr, crop_border=0)


    psnr_gt = pnsr(gt, gan).numpy()[0]
    ssim_gt = float(ssim(gt, gan).numpy())
    fsim_gt = fsim(gt, gan).numpy()[0]
    ms_ssim_gt = float(msssim(gt, gan).numpy())
    ...
    piqe_gt = basicsr.metrics.calculate_niqe(gt, crop_border=0)
    piqe_gan = basicsr.metrics.calculate_niqe(gan, crop_border=0)
